from bs4 import BeautifulSoup
import random
import time
from datetime import datetime
import pandas as pd
import re
import logging
import os
from scraping_scripts.headers import headers, user_agents
from scraping_scripts.utils import get_with_retries

#Tracks categories to prevent redundant CSV headers.
processed_categories = set()

is_error = False

data = []

# extracts a single product data and saves it
def extract_and_save_data(html: str, category: str):
    global processed_categories
    soup = BeautifulSoup(html, "html.parser")
    title = soup.select_one("h1[class='-fs20 -pts -pbxs']")
    title = title.text.strip() if title else None

    # Exclude irrelevant products in the "tablets" category
    if title and not (category == "tablets" and not re.search("pad|tab", title, re.IGNORECASE)):
        price = soup.select_one("[class='-b -ubpt -tal -fs24 -prxs']")
        price = price.text.strip() if price else None

        discount = soup.select_one("[class='bdg _dsct _dyn -mls']")
        discount = "discount" if discount else "no discount"

        data.append({
            "name": title,
            "price": price,
            "website": "aliexpress",
            "category": category,
            "scraping_date": datetime.now().date(),
            "promotion": discount,
        })


def scrape_data(url: str, headers: dict, category: str, max_pages: int):
    global is_error
    global data

    page_number = 1
    visited_urls = set()

    # Load previously visited URLs (if available)
    if os.path.exists(f"cache/aliexpress_visited_{category}_urls.txt"):
        with open(f"cache/aliexpress_visited_{category}_urls.txt", "r") as f:
            visited_urls = set(map(lambda url: url.strip("\n"), f.readlines()))

    while True:
        try:
            response = get_with_retries(url, headers=headers)
            if response.status_code != 200:
                is_error = True
                print(f"Error fetching {url}: Status code {response.status_code}")
                break
            
            soup = BeautifulSoup(response.text, "html.parser")
            products_list = soup.select_one('div[id="card-list"]')
            # products_list = products_list if products_list else soup.select_one("[class='prd-w col8 row _2c -paxs']")
            if not products_list:
                # logging.error(soup.prettify())  # Print the first 1000 characters of the response
                print(f"No products found on page {page_number} for {category} - {url=}")
                break

            a_tags = products_list.select('a[class="multi--container--1UZxxHY cards--card--3PJxwBm cards--list--2rmDt5R search-card-item"]')
            headers["Referer"] = url  # Update Referer header
            
            for a_tag in a_tags:
                a_tag_href = f"https:{a_tag.get("href").strip()}" if a_tag.get("href") else ""

                if a_tag_href in visited_urls:
                    print(f"Skipping already visited page: {a_tag_href}")
                    continue

                time.sleep(random.uniform(4, 8))
                headers["User-Agent"] = random.choice(user_agents)
                page = get_with_retries(a_tag_href, headers=headers)
                if page.status_code == 200:
                    visited_urls.add(a_tag_href)
                    extract_and_save_data(html=page.text, category=category)

            # Save data to CSV
            df = pd.DataFrame(data)
            header = not (category in processed_categories)
            df.to_csv(f"data/aliexpress_{category}.csv", index=False, mode="a", header=header)
            processed_categories.add(category) # ensure headers are only written once
            data.clear()

            # Move to the next page
            page_number += 1
            if page_number > max_pages:
                break
            url = url.replace(f"?page={page_number-1}", f"?page={page_number}")

        except Exception as e:
            is_error = True
            current_time = datetime.now()
            print(f"Error while scraping aliexpress, page {page_number} of {category}:")
            print(e)
            logging.error(f"{current_time} : Error on page {page_number} of {category}: {url}:")
            logging.error(e)
            break

    # Save visited URL
    with open(f"cache/aliexpress_visited_{category}_urls.txt", "a") as f:
        f.writelines(f"{url}\n" for url in visited_urls)


def main():
    logging.basicConfig(filename="logs/aliexpress_scraping_errors.log", level=logging.ERROR)

    scraping_targets = [
        {
            "category": "smartphones",
            "url": "https://www.aliexpress.com/w/wholesale-smartphone.html?page=1&g=n&SearchText=smartphone",
            "max_pages": 12,
        },
        {
            "category": "laptops",
            "url": "https://www.aliexpress.com/w/wholesale-laptops.html?page=1&g=n&SearchText=laptops",
            "max_pages": 9,
        },
        {
            "category": "tablets",
            "url": "https://www.aliexpress.com/w/wholesale-tablets.html?page=1&g=n&SearchText=tablets",
            "max_pages": 2,
        }
    ]

    for target in scraping_targets:
        if os.path.exists(f"data/aliexpress_{target["category"]}.csv"): # if file exists:
            with open(f"data/aliexpress_{target["category"]}.csv", mode="r") as file:
                if file.read(1): # if file is not empty
                    # ensure that the headers are not written to the csv file as it is not empty
                    processed_categories.add(target["category"])

        scrape_data(target["url"], headers=headers, category=target["category"], max_pages=target["max_pages"])

        # if no error, then the scraping finished successfully, truncate the cache/aliexpress_visited_urls.txt
        if not is_error:
            with open(f"cache/aliexpress_visited_{target["category"]}_urls.txt", "w") as f:
                pass
            print(f"scraping data finished for category '{target["category"]}' in aliexpress")
        else:
            print(f"something went worng when scraping data for category '{target["category"]}' in aliexpress, please check log and cache files")

        print(f"scraping data finished for category '{target["category"]}' in aliexpress")

    print("scraping data finished for aliexpress")


if __name__ == "__main__":
    if not os.path.exists("./data"):
        os.mkdir("./data")

    if not os.path.exists("./cache"):
        os.mkdir("./cache")

    main()